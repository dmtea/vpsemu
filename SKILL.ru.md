...
version: 0.33
description: >
  Управление LXD-контейнерами которые имитируют VPS-серверы (Ubuntu 24.04).
...
# Настроить SSH и зависимости — пароль НЕ устанавливается здесь
lxc exec vps-template -- bash -c "
  # Ubuntu 24.04 использует sshd_config.d/ — переопределяем через отдельный файл
...
# Зафиксировать чистое состояние
...
3. Никогда не использовать `lxc delete` без полного явного `${NAME}`.
4. Никогда не трогать `vps-template` — только источник для `lxc copy`.
